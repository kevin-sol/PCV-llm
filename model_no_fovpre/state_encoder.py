...
class EncoderNetwork(nn.Module):
    ...
    def __init__(self, embed_dim=128):
        ...
        self.output_layer = nn.Sequential(
            nn.Linear(embed_dim * 6, embed_dim),  # 从5变为6，增加了距离特征
    ...
    def forward(self, state):
        ...
        fov_length = 50 * TILE_IN_F  # 历史FOV长度 * tile数量
        fov_end = min(fov_start + fov_length, feature_dim)
        
        # 重组FOV特征为适合卷积处理的格式 [batch_size, TILE_IN_F, history_len]
        if fov_end > fov_start:
            fov_flat = state[:, -1, fov_start:fov_end]  # [batch_size, fov_length]
            
            # 确保长度正确
            if fov_flat.shape[1] < fov_length:
                padding = torch.zeros(batch_size, fov_length - fov_flat.shape[1], device=state.device)
                fov_flat = torch.cat([fov_flat, padding], dim=1)
                
            # 重塑为 [batch_size, TILE_IN_F, 50]
            fov_features = fov_flat.reshape(batch_size, TILE_IN_F, 50)
        else:
            # 如果没有FOV特征，创建零张量
            fov_features = torch.zeros(batch_size, TILE_IN_F, 50, device=state.device)
        ...
        # 帧计数器特征
        frame_counter_idx = gof_end
        frame_counter = state[:, -1, frame_counter_idx:frame_counter_idx+1] if frame_counter_idx < feature_dim else torch.zeros(batch_size, 1, device=state.device)
        
        # 距离历史特征 - 在帧计数器之后
        distance_start = frame_counter_idx + 1
        distance_length = 50 * TILE_IN_F  # 历史距离长度 * tile数量
        distance_end = min(distance_start + distance_length, feature_dim)
        
        # 重组距离特征为适合卷积处理的格式 [batch_size, TILE_IN_F, history_len]
        if distance_end > distance_start:
            distance_flat = state[:, -1, distance_start:distance_end]  # [batch_size, distance_length]
            
            # 确保长度正确
            if distance_flat.shape[1] < distance_length:
                padding = torch.ones(batch_size, distance_length - distance_flat.shape[1], device=state.device)  # 默认距离为1
                distance_flat = torch.cat([distance_flat, padding], dim=1)
                
            # 重塑为 [batch_size, TILE_IN_F, 50]
            distance_features = distance_flat.reshape(batch_size, TILE_IN_F, 50)
        else:
            # 如果没有距离特征，创建全1张量（默认距离为1）
            distance_features = torch.ones(batch_size, TILE_IN_F, 50, device=state.device)
        ...
        encoded_distance = self.distance_encoder(distance_features)  # [batch_size, embed_dim]
