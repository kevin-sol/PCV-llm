...
class LlamaPointCloudController:
    ...
    def stream_next_gof(self, user_fov_trace, user_dis_trace, current_frame):
        ...
        with torch.no_grad():
            ...
            # 获取质量级别决策
            quality_levels = self.policy.sample(
                state=state,
        ...
        # 从质量级别决策确定要下载哪些tile
        # # 如果质量级别>0，则下载该tile
        # selected_tile = np.zeros(TILE_IN_F, dtype=int)
        # for i in range(TILE_IN_F):
        #     if quality_levels[i] > 0:
        #         selected_tile[i] = 1
        selected_tile = np.ones(TILE_IN_F, dtype=int)
        # 执行下载
